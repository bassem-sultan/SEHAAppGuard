 ...
 */
public class SEHAAppGuard extends CordovaPlugin {
    private static final String TAG = "SEHAAppGuard";
    ...
    private boolean checkSuCommand() {
        // 1) Try plain `which su` (no absolute path) — runCommandWithTimeout will
        ...
        for (String path : suPaths) {
            try {
                ...
                // If checking file existence causes a security exception, treat as suspicious
                // (fail-safe)
    ...
    private boolean checkSystemMount() {
        ...
        try {
            ...
            // If mount binary is missing or cannot run, that alone isn't proof of root; but
            // treat other IO issues as suspicious.
            String msg = e.getMessage() != null ? e.getMessage() : "";
            log("Error checking mount: " + msg);
            // If it's "No such file or directory" specifically, don't assume compromised
            // just for that.
    ...
    // ---------------------------
    /**
     * Run a command with a timeout. Returns true if the command produced output
     * (meaning suspicious for checks like `which su` / `pidof`),
     * false if no output or the binary is missing.
     *
     * Behavior:
     * - If process cannot be started because binary is missing (IOException
     * containing error=2 / No such file) -> return false.
    ...
    private boolean runCommandWithTimeout(String[] command, int timeoutMs) {
        ...
        try {
            ...
            // If binary is missing on the system (error=2 / No such file or directory),
            // treat as NOT compromised
            if (msg.contains("error=2") || msg.toLowerCase().contains("no such file")) {
                return false;
            }
